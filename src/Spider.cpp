...
using namespace std;
	...
	void Spider::initLegs(){
		...
		//rightLeg[1].joint[2] = createPoint();
		...
		//rightLeg[2].joint[2] = createPoint();
		...
		//rightLeg[3].joint[2] = createPoint();

	}

	// desenha uma perna
	void drawLeg(Leg l){
		for(int i=0;i<2;i++)	glVertex3d(l.joint[i].x,l.joint[i].y,l.joint[i].z); // tem que aumentar para 3 depois
	}
